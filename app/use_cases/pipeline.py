...
from app.domain.zones import classify_zone
...
class ValidateEligibilityStep:
    ...
    async def execute(self, ctx: QuoteContext) -> None:
        r = ctx.request
        ctx.package = build_package(r.weight_kg, r.length_cm, r.width_cm, r.height_cm, r.declared_value_ars)
        ctx.tracer.mark(
            "dominio",
...
class ClassifyZoneStep:
    ...
    async def execute(self, ctx: QuoteContext) -> None:
        ...
        ctx.tracer.mark("dominio", "ClassifyZoneStep", f"CP {ctx.request.postal_code} -> zona {ctx.zone}")
